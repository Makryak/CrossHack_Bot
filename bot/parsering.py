...
def get_sheet_formatting(service, sheet_id, sheet_name):
    try:
        result = service.spreadsheets().get(spreadsheetId=sheet_id, ranges=f'{sheet_name}!A:AZ', fields="sheets(data(rowData(values(userEnteredFormat))))").execute()
        return result
    except HttpError as error:
        print(f'An error occurred: {error}')
        return None
...
def parse_skills_data(values, formatting):
    ...
    for row_index, row in enumerate(values[1:], start=1):
        ...
        for col_index in range(2, len(headers)):
            try:
                cell_format = formatting['sheets'][0]['data'][0]['rowData'][row_index]['values'][col_index]['userEnteredFormat']
